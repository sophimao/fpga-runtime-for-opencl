...
void l_get_physical_devices(acl_mmd_dispatch_t *mmd_dispatch,
  ...
  int num_boards;
  char *ptr, *saveptr;
  // This is a bit subtle, pointers to device names might get
  // cached by various routines
  ...
  // TODO: Let's make calling sim device creation function in between
  //       context creation giving undefined behaviour for now :P
...
#define MAX_PATH_LEN 512
  ...
  for (unsigned iboard = 0; iboard < num_board_pkgs; ++iboard) {
    l_get_physical_devices(
        &(internal_mmd_dispatch[iboard]), num_physical_devices);
